...
# GitHub's latest/download redirect resolves to the newest release's asset with
# no API call, so it avoids the unauthenticated API rate limit and JSON parsing.
# Override with a local zip for testing: ASSET_URL=file:///path/to.zip
